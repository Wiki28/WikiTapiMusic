...
async def play(_, message: Message):
    ...
    if message.sender_chat:
        return await message.reply_text(
            "kamu adalah __Admin Anonim or Akun Channel__ di Grup Obrolan ini!\nRkembali ke Akun Pengguna."
        )
    audio = (
    ...
    if audio:
        ...
        try:
            ...
            if read:
                return await mystic.edit(
                    "Pemutaran Streaming Langsung...Hentikan untuk memutar musik"
                )
            else:
                pass
        except:
        ...
        mystic = await message.reply_text("🔄 Pengolahan URL... Harap tunggu!")
...
async def slider_query_results(_, CallbackQuery):
    ...
    if what == "F":
        ...
        med = InputMediaPhoto(
            media=thumb,
            caption=f"📎Judul: **{title}\n\n⏳Durasi:** {duration_min} Mins\n\n__[Dapatkan Informasi Tambahan Tentang Videol](https://t.me/{BOT_USERNAME}?start=info_{videoid})__",
        )
        return await CallbackQuery.edit_message_media(
            media=med, reply_markup=InlineKeyboardMarkup(buttons)
        )
