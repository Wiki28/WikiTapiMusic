...
async def set_video_limit_kid(_, message: Message):
    ...
    await message.reply_text(
        f"Batas Maksimum Panggilan Video Ditetapkan ke {limit} Obrolan."
    )
...
async def logger(_, message):
    if LOG_SESSION == "None":
        return await message.reply_text(
            "Tidak Ada Akun Pencatat yang Ditentukan.\n\nSilakan Setel <code>LOG_SESSION</code> var lalu coba masuk."
        )
    usage = "**Usage:**\n/logger [enable|disable]"
    if len(message.command) != 2:
        return await message.reply_text(usage)
...
async def ban_globally(_, message):
    if not message.reply_to_message:
        ...
        if user.id == from_user.id:
            return await message.reply_text(
                "Anda ingin nge-gban sendiri? Betapa bodohnya!"
            )
        elif user.id == BOT_ID:
...
**Pengguna yang Diblokir ID:** `{user.id}`
**Obrolan:** {number_of_chats}"""
    ...
    if user_id == from_user_id:
        ...
        await message.reply_text("Haruskah saya memblokir diri saya sendiri?? Lmao Ded!")
...
async def unban_globally(_, message):
    if not message.reply_to_message:
        ...
        if user.id == from_user.id:
            ...
            await message.reply_text("Pengguna Sudo tidak bisa blocked/unblocked.")
        else:
...
async def broadcast_message_pin_silent(_, message):
    if not message.reply_to_message:
        ...
        for i in chats:
            try:
                ...
                pass
        await message.reply_text(
            f"**Pesan Siaran Dalam {sent}  Mengobrol dengan {pin} pin.**"
        )
        return
    if len(message.command) < 2:
        await message.reply_text(
    ...
    for i in chats:
        try:
            ...
            pass
    await message.reply_text(
        f"**Pesan Siaran Dalam {sent} Obrolan dan {pin} pin.**"
    )
    ...
    if not message.reply_to_message:
        ...
        for i in chats:
            try:
                ...
                pass
        await message.reply_text(f"**Pesan Siaran Dalam {sent} Obrolan.**")
        return
    if len(message.command) < 2:
        await message.reply_text(
            "**Usage**:\n/broadcast [MESSAGE] or [Reply to a Message]"
...
async def clean(_, message):
    ...
    os.mkdir(dir)
    os.mkdir(dir1)
    await message.reply_text("Berhasil membersihkan semua **suhu** dir(s)!")
